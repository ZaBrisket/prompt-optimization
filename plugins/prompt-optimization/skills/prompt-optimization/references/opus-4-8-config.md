...
| **Residual long-horizon failure modes** (system card §2.3.3): fabrication, ignored correction, cheap-verification-skipped, goal drift, subagent fork-one-directive, recap hallucination. | These persist in long multi-turn agentic runs *alongside* the controlled-single-shot honesty gains above — so completion-verification and confirmation guardrails stay. Behavioral detail in [opus-4-8-system-card.md](opus-4-8-system-card.md) §3; prompt-actionable rules in `task-heuristics.md` rows 7 and 32–35. |
...
- **Honesty:** 4.8 is materially more calibrated than 4.7 on controlled single-shot diligence (strongest measured model on the uncritical-reporting, lazy-investigation, and overconfidence evals). The completion-verification guardrail (Meta-Rule 11) remains as defense-in-depth — not because the model is unreliable in general, but because the system card's §2.3.3 long-horizon failure modes persist in long multi-turn agentic runs. See [opus-4-8-system-card.md](opus-4-8-system-card.md) §2–§3 for the eval numbers and the failure-mode list (the single home for both, so there is one source to re-verify).
- **Refusal calibration:** 4.8 refusals are longer and more elaborate than 4.7, and over-refusal is slightly *higher*, not lower (4.8 engages opposing perspectives on contested topics more readily — distinct from refusing less). For API targets where refusal length matters, an optional "keep refusals concise" system-prompt directive mirrors Anthropic's own claude.ai mitigation. See [opus-4-8-system-card.md](opus-4-8-system-card.md) §5; `task-heuristics.md` row 31.
- **Safety nuance:** the 4.8 system card reports it is somewhat *less* robust to prompt injection than 4.7 and more willing to start a task without scrutinizing intent (a documented malicious-computer-use refusal regression). Injection risk concentrates on browser-use and computer-use surfaces and on the data-access + action-taking combination, and extended thinking does not uniformly help; the Claude Code FileRead malicious-content reminder was removed. Product-level safeguards close the gap in deployed products but are not present in raw API / third-party harnesses. For agentic and dynamic-workflow deliverables — where workflow-spawned agents run in `acceptEdits` mode — keep reversibility and confirmation guardrails strong and treat ingested content as data, not instructions (Meta-Rule 11, Meta-Rule 15; `task-heuristics.md` rows 8/29/35; [opus-4-8-system-card.md](opus-4-8-system-card.md) §6).
...
- [Claude Opus 4.8 System Card](https://www.anthropic.com/claude-opus-4-8-system-card) — honesty/calibration gains and prompt-injection robustness nuance; §2.3.3 residual long-horizon failure modes, §4 eval/grader awareness, §5 refusal calibration, §6 agentic-safety regressions, §8 capabilities. Behavioral detail and the volatile numbers are routed through [opus-4-8-system-card.md](opus-4-8-system-card.md).
