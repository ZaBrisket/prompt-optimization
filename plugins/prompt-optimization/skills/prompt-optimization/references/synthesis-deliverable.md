...
- **The overall conclusion is checked against the mission, not an intermediate milestone.** The Opus 4.8 system card (§2.3.3) documents the model treating an intermediate milestone (a lane returning "done") as the finish line. Before finalizing, the reduce step compares the overall conclusion to the orchestrator's `## Mission` statement and confirms the conclusion answers the mission — not a subordinate question that surfaced along the way. (`opus-4-8-system-card.md` §3.)
- **Lane uncertainty is preserved, not dropped.** The system card (§2.3.3) documents 4.8 dropping a subagent's caveat and relaying a "could not verify" guess as a confirmed fact. The reduce step carries each lane's caveats verbatim and treats "the lane could not verify X" as a negative result that lowers the key finding's confidence (or strikes it), never as a fact to relay confidently. This is the upstream complement to the source-validation revisit pass. (`opus-4-8-system-card.md` §3.)
